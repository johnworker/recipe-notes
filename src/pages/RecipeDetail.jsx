  ...
  if (!recipe) {
    return (
      <main className="container py-14">
        ...
        <div className="text-center mt-4"><Link className="underline" to="/recipes">回食譜列表</Link></div>
      </main>
    );
  }

  const isFav = favorites.includes(recipe.id);
  const ingredients = recipe.ingredients?.length ? recipe.ingredients : ['鹽少許','黑胡椒少許','橄欖油 1 大匙','蒜末 1 瓣'];
  const seasonings  = recipe.seasonings?.length ? recipe.seasonings : [];

  return (
    <main className="container py-8 space-y-10">
      <Breadcrumbs trail={[{ to: '/recipes', label: '食譜' }, { label: recipe.title }]} />

      {/* Hero：大圖置中（上方多留白 → 往下移） */}
      <section className="space-y-5">
        <div className="mx-auto w-full max-w-[520px] aspect-square relative rounded-2xl overflow-hidden
          ...
          />
          <LightboxModal open={open} src={recipe.image} alt={recipe.title} onClose={()=>setOpen(false)} />
          <button
            onClick={()=>toggleFavorite(recipe.id)}
            className={`absolute top-3 right-3 rounded-full px-3 py-1.5 text-sm transition
                        ${isFav ? 'bg-rose-600 text-white' : 'bg-white/90 dark:bg-gray-900/70 border dark:border-gray-700'}`}
          >
            {isFav ? '已收藏 ❤️' : '加入收藏 🤍'}
          </button>
        </div>
        ...
        <div className="max-w-3xl mx-auto text-center space-y-4">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <button onClick={()=>window.print()} className="btn-ghost no-print">列印</button>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto">
        <div className="flex flex-wrap gap-2 justify-center">
          {(recipe.tags || []).map(t => (
            <span key={t} className="px-2 py-1 text-sm rounded-full border bg-white dark:bg-gray-800 dark:border-gray-700">#{t}</span>
          ))}
        </div>
      </section>
      ...
      {related.length > 0 && (
        <section class="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {related.map(r => (
              <Link key={r.id} to={`/recipes/${r.id}`} className="border rounded-xl overflow-hidden bg-white dark:bg-gray-800 dark:border-gray-700 hover:shadow transition">
                <img src={r.image} alt={r.title} className="w-full h-28 object-cover" />
                <div className="p-3">
                  <div className="font-semibold line-clamp-1">{r.title}</div>
                  <div className="text-xs text-gray-500">{(r.tags || []).slice(0,2).map(t=>`#${t}`).join(' ')}</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div class="max-w-5xl mx-auto">
        <Link to="/notes/edit" state={{ recipeId: recipe.id }} className="btn-secondary">為此食譜新增筆記</Link>
      </div>
    </main>
  );
}
