...
import { useAuth } from "./AuthProvider"; // 有照我前一步建立 AuthProvider 就引入
...
const initialRecipes = [
  ...
  },
  {
    ...
    seasonings: [
      "凱撒醬(蛋黃/美乃滋, 橄欖油, 蒜, 鯷魚, 檸檬汁, 第戎芥, 醬油或伍斯特醬, 黑胡椒)",
    ],
...
export function RecipeProvider({ children }) {
  ...
  const { user } = useAuth(); // 目前登入者
  const [favoritesByUser, setFavByUser] = useLocalStorage(
    "favorites_by_user",
    {}
  ); // {userId: [recipeId]}
  ...
  useEffect(() => {
    const repaired = recipes.map((r) => {
      const needFix =
        !r.image ||
        (typeof r.image === "string" && r.image.startsWith("/src/"));
      return needFix ? { ...r, image: IMAGE_BY_ID[r.id] || salad } : r;
    });
    if (JSON.stringify(repaired) !== JSON.stringify(recipes))
      setRecipes(repaired);
  ...
  const toggleFavorite = (id) => {
    ...
    setFavByUser((prev) => {
      const list = new Set(prev[userId] || []);
      list.has(id) ? list.delete(id) : list.add(id);
      return { ...prev, [userId]: Array.from(list) };
    });
    // 小提示（非必要）：立即回饋
    // toast.success(userFavorites.includes(id) ? '已移除收藏' : '已加入收藏');
  };
  ...
  return (
    <RecipeContext.Provider
      value={{
        ...
        notes,
        setNotes,
        favorites: userFavorites, // ✅ 對外仍叫 favorites
        toggleFavorite,
        createRecipe,
        updateRecipe,
        deleteRecipe,
